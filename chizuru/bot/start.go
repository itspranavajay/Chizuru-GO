...
func startCMD(b *gotgbot.Bot, ctx *ext.Context)(err error){
	if ctx.Message.Chat.Type == "private" {
		_, err = ctx.EffectiveMessage.Reply(b, fmt.Sprintf("Hello, I'm %s, I fetch lewds just for you >/////<", b.User.FirstName), &gotgbot.SendMessageOpts{
			ParseMode: "html",
			ReplyMarkup: gotgbot.InlineKeyboardMarkup{
				InlineKeyboard: [][]gotgbot.InlineKeyboardButton{{
					{Text: "Channel", Url: config.ChannelLINK},
					{Text: "Chat", Url: config.ChannelLINK},
				}},
			},
		})
	} else {
